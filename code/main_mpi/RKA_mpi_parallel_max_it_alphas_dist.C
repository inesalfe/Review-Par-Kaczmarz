...
// mpic++ -o bin/RKA_mpi_parallel_max_it_alphas_dist.exe main_mpi/RKA_mpi_parallel_max_it_alphas_dist.C
// mpirun -np 2 ./bin/RKA_mpi_parallel_max_it_alphas_dist.exe dense 1 1E-10 2000 100 20
...
void importAlpha(int M, int N, int np, int id, double& alpha) {

	ifstream file_alpha;
	file_alpha.open("outputs/mpi/RKA_best_alpha_tasks_dist.txt");
	...
	string line;
	string target_line = to_string(M) + " " + to_string(N) + " " + to_string(np);
...
int main (int argc, char *argv[]) {
	...
	if(argc != 6) {
		...
		cout << "'mpirun -np <nproc> ./bin/RKA_mpi_parallel_max_it_alphas_dist.exe <data_set> <n_runs> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	double res_entry;
	double sqrNorm_res_partial = 0;
	double sqrNorm_res = 0;
	for (int i = 0; i < M_size; i++) {
		res_entry = b[i] - dotProduct(A[i], x_sol, N);
		sqrNorm_res_partial += res_entry*res_entry;
	}
	
	MPI_Allreduce(&sqrNorm_res_partial, &sqrNorm_res, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
