...
int main (int argc, char *argv[]) {
	...
	if(argc != 9) {
		...
		cout << "'mpirun -np <nproc> ./bin/RKAB_mpi_max_it_alpha_dist.exe <data_set> <n_runs> <M> <N> <nproc> <block_size> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(it < max_it_stop) {
			...
			for (int i = 0; i < nproc; i++) {
				...
				for (int j = 0; j < N; j++) {
					x_k_proc[j] = x_prev[j] + scale * A[line][j];
				}
				for (int k = 1; k < block_size-1; k++) {
					line = BLOCK_LOW(i, nproc, M)+dist[i](gen[i]);
					scale = alpha * (b[line]-dotProduct(A[line], x_k_proc, N))/sqrNorm_line[line];
					for (int j = 0; j < N; j++) {
						x_k_proc[j] += scale * A[line][j];
					}
				}
				line = BLOCK_LOW(i, nproc, M)+dist[i](gen[i]);
				scale = alpha * (b[line]-dotProduct(A[line], x_k_proc, N))/sqrNorm_line[line];
				for (int j = 0; j < N; j++) {
					x_k_proc[j] += scale * A[line][j] - x_prev[j];
				}
				for (int j = 0; j < N; j++) {
					x_k[j] += x_k_proc[j]/nproc;
				}
			}
		}
		stop = MPI_Wtime();
