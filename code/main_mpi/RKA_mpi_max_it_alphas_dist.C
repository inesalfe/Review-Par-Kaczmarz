...
void importAlpha(int M, int N, int nproc, double*& alpha) {
	...
	alpha = new double[nproc];

	string line;
	string target_line = to_string(M) + " " + to_string(N) + " " + to_string(nproc);
...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'mpirun -np <nproc> ./bin/RKA_mpi_max_it_alphas_dist.exe <data_set> <n_runs> <M> <N> <nproc> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(it < max_it_stop) {
			...
			for (int i = 0; i < nproc; i++) {
				...
				scale = alpha[i] * (b[line]-dotProduct(A[line], x_prev, N))/sqrNorm_line[line];
				for (int j = 0; j < N; j++) {
