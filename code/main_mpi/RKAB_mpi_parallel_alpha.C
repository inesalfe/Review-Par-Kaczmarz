...
// mpic++ -o bin/RKAB_mpi_parallel.exe main_mpi/RKAB_mpi_parallel.C
// mpirun -np 2 ./bin/RKAB_mpi_parallel.exe dense 1 1E-10 2000 100 20
...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'mpirun -np <nproc> ./bin/RKAB_mpi_parallel_alpha.exe <data_set> <n_runs> <eps> <M> <N> <block_size> <alpha>'" << endl;
		exit(1);
	}
