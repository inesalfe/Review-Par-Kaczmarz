...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'./bin/RKAB_parallel_max_it_3.exe <data_set> <n_runs> <M> <N> <block_size> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel num_threads(block_threads) private(t_id_out, x_k_thread) firstprivate(it)
		{
			...
			while(it < max_it_stop) {
				...
				// Não é preciso a barreira de baixo por causa do for na linha 131 (tem barreira implícita)
				// #pragma omp barrier
				#pragma omp critical
					for (int i = 0; i < N; i++)
