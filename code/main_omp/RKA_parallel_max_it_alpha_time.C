...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'./bin/RKA_parallel_max_it_alpha_time.exe <data_set> <n_runs> <M> <N> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	double talk_time_start; 
	double talk_time_end;
	double talk_time_duration = 0;

	double comp_time_start; 
	double comp_time_end;
	double comp_time_duration = 0;
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id) firstprivate(it)
		{
			...
			while(it < max_it_stop) {
				...
				#pragma omp single
				{
					talk_time_end = omp_get_wtime();
					talk_time_duration += talk_time_end-talk_time_start;
					comp_time_start = omp_get_wtime();
				}
				#pragma omp for
					for (int i = 0; i < N; i++)
						x_prev[i] = x_k[i];
				...
				#pragma omp single
				{
					comp_time_end = omp_get_wtime();
					comp_time_duration += comp_time_end-comp_time_start;
					talk_time_start = omp_get_wtime();
				}
				#pragma omp critical
					for (int i = 0; i < N; i++)
	...
	cout << sqrNormDiff(x_sol, x, N) << " " << duration_total << " " << comp_time_duration << " " << talk_time_duration << endl;
